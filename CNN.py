    ...
    def summary(self):
        ...
        for layer in self.layers:
            ...
            if (layer.layer_name=='conv2d')|(layer.layer_name=='fullyconnect'):
                ...
                print('name:',layer.layer_name,'   input_shape:',layer.shape,'   output_shape:',layer.output_shape,'  weights:',layer.weights.shape,'bias',layer.bias.shape)

                bytes=1

                for i in layer.weights.shape:

                    bytes=bytes*i

                bytes=bytes*4

                print('预计占用内存量为:',bytes/1024,'k')
